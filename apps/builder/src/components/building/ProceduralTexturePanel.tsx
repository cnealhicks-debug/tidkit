...
import type { ProceduralTextureConfig, BrickBondPattern, StoneLayout } from '@/types/procedural';
...
export function ProceduralTexturePanel({ surface, onClose }: ProceduralTexturePanelProps) {
  const { textures, setProceduralTexture, updateProceduralParam, applyProceduralPreset } = useBuildingStore();
  const [typeFilter, setTypeFilter] = useState<'brick' | 'stone'>('brick');
  const [showWeathering, setShowWeathering] = useState(false);
  ...
  const handleApplyPreset = (presetConfig: ProceduralTextureConfig) => {
    applyProceduralPreset(surface, presetConfig);
  };

  const handleSetType = (type: 'brick' | 'stone') => {
    setTypeFilter(type);
    if (!config || config.type !== type) {
      setProceduralTexture(surface, createDefaultConfig(type));
    }
  };
          ...
          {filteredPresets.map((preset) => (
            <button
              ...
              className={`px-2 py-2 text-xs rounded-lg border text-left transition-colors ${
                config && JSON.stringify(config) === JSON.stringify(preset.config)
...
function BrickControls({ brick, updateParam }: { brick: NonNullable<ProceduralTextureConfig['brick']>; updateParam: (path: string, value: any) => void }) {
...
function StoneControls({ stone, updateParam }: { stone: NonNullable<ProceduralTextureConfig['stone']>; updateParam: (path: string, value: any) => void }) {
...
function WeatheringControls({ weathering, updateParam }: { weathering: ProceduralTextureConfig['weathering']; updateParam: (path: string, value: any) => void }) {
  return (
